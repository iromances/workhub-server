Create Codex pet row `running-right` for `qianduoduo`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 严格保留现有钱多多：金色短毛圆脸猫，金棕色毛、浅色肚皮、圆眼睛、黑尾尖；以现有 8x9 图集为唯一身份和风格来源，只补充自然注视方向。. Preserve silhouette, face, palette, material, proportions, markings, and props.

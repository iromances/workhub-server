Create one horizontal look-direction strip for Codex pet `qianduoduo`, atlas row 9.
...
REPAIR LOCK FOR CROSS-ROW CONTINUITY: the prior attempt failed because the whole reclining body progressively rotated and `157.5` became too horizontal, causing a conspicuous snap into the approved `180 down` cardinal. Keep the entire body pose, belly, rear paws, forepaws, tail root, black tail tip, scale, and lower-body anchor nearly fixed across all eight cells. Carry direction primarily through whole-eyeball rotation, eyelids, nose/muzzle, and a small head turn; ears and whiskers follow subtly. Use the approved `180 down` cardinal as the body-registration target for the end of this row. `157.5` must be almost the same body pose, size, center, and baseline as `180`, differing only by one remaining small screen-right eye/head component. Do not make the cat progressively lie flatter, rotate, roll, or shift right across the row.

Create one horizontal look-direction strip for Codex pet `qianduoduo`, atlas row 10.
...
REPAIR LOCK FOR CROSS-ROW CONTINUITY: a previous candidate exposed a large `157.5 -> 180` center and area jump. Use the newly repaired row 9 as the exact body-registration source. Keep the entire body pose, belly, rear paws, forepaws, tail root, black tail tip, scale, and lower-body anchor nearly fixed across all eight cells. Carry direction primarily through whole-eyeball rotation, eyelids, nose/muzzle, and a small head turn; ears and whiskers follow subtly. `180` must match repaired `157.5` in body size, center, baseline, and construction, differing only by the final one-step removal of the screen-right gaze component. Do not rotate, roll, progressively stand up, or laterally shift the whole cat.

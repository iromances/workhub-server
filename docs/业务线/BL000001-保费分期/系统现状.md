...
本文承接《保费分期系统生产 master 反向产品与流程梳理》的既有内容。2026-09-17 仅整理文档路径并通过 WorkHub 核对业务线编号和名称，未重新核验业务实现；原核验日期、分支与证据限制保留如下。

业务线附件：[保费分期产品要素.xlsx](保费分期产品要素.xlsx)。正式需求文档按[业务线文档指引](../AGENTS.md)存放于本目录下的 `{需求编号}-{需求名称}/`。
...
| 环境 | 生产 |
| 分支 | master |
| 代码工作区 | `/Users/aslight/IDEAWorkspace/scf` |

本次按 WorkHub 业务线规则，限定在 `/Users/aslight/IDEAWorkspace/scf` 下 Git remote 明确属于 `scf` 组的生产 master 代码进行反向梳理，未使用其他业务线目录中的相似仓库或历史经验替代结论。

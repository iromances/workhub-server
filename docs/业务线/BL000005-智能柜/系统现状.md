...
当前资料覆盖范围为“买断价两套计算方式与三项目三场景”，尚非业务线全量系统梳理。本文保留既有分析；2026-09-17 仅整理文档路径并通过 WorkHub 核对业务线编号和名称，未重新核验业务实现，原核验日期、分支与证据限制保留如下。

第 3 节目标口径、第 12—15 节差距、建议设计、验收矩阵和实施顺序属于历史需求参考，不代表当前实现、已确认规格或已执行验证。后续确认系统中的需求编号和名称后，正式需求文档按[业务线文档指引](../AGENTS.md)写入本目录下的 `{需求编号}-{需求名称}/`，再依据实际验证结果更新现状。
...
本次工具环境未暴露生产数据库和生产配置只读 MCP 资源，因此按业务运维分析规则使用 `/Users/aslight/IDEAWorkspace/ol` 下 Git remote 明确属于 `ol` 组、当前分支为 `master` 且工作区干净的仓库进行源码核对。

...
4. 更新接口类型和文档。
   - 交付内容：前端类型保持与后端响应一致；后端 `docs/事实/控制器接口说明.md` 更新页面使用说明和接口关系。
   - 验收标准：文档说明项目用途绑定已作为商户详情下的维护区块；接口仍保留兼容调用。
   - 建议禅道标题：`【支付配置】更新绑定合并后的接口和页面文档`
   - 内聚改动点：`workhub-web/src/types/payment.ts`、`workhub-web/src/api/payment.ts`、`workhub-server/docs/事实/控制器接口说明.md`。
...
- `workhub-web`
  ...
  - 相关专题文档：如页面能力描述需要同步，更新支付配置页说明。
- `workhub-server`
  - `docs/事实/控制器接口说明.md`：更新支付配置接口使用说明，说明新增绑定入口合并到编辑商户弹窗。

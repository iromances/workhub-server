...
| `workhub-bootstrap/src/main/resources/db/schema/mysql/` | 否 | 应用初始化 SQL。 |
| `dbscript/DDL/` | 否 | 按需求归档的数据库结构变更脚本。 |
| `dbscript/DML/` | 否 | 按需求归档的数据修正、数据初始化或历史回填脚本。 |
...
- `SERVER` 资源通过业务线下的系统列表 `systemNames` 支持同一服务器目标承载一个或多个微服务系统；兼容字段 `systemName` 表示首个系统。
